                ...
                try:
                    ...
                    with io.open(file["path"], "rb") as file_object:
                        ...
                        if len(response["errors"]) == 0:
                            ...
                            submissions.append({"submission_id": submission_id, "sample_id": sample_id,
                                                "is_truncated": file["is_truncated"], "sha256": file["sha256"]})
                            self.log.debug("File %s submitted to VMRay" % file["path"])
                        else:
    ...
    def wait_submissions(self, submissions):
        ...
        for submission in submissions:
            submission_objects.append({"submission_id": submission["submission_id"],
                                       "is_truncated": submission["is_truncated"],
